...
import { X, Earth, Lock, User, Check, Plus, Eye, EyeOff, Calendar } from 'lucide-react'
...
export default function SchedulesAndAvailability({ storeId, storeSlug }: SchedulesAndAvailabilityProps) {
    ...
    const loadToggle = useCallback(async () => {
        ...
        const { data } = await supabase
            ...
            .single()
        if (data) {
            setAllowScheduling(data.allow_scheduling ?? true)
        }
    }, [storeId])
    ...
    const handleToggleChange = async (checked: boolean) => {
        ...
        const { error } = await supabase
            .from('stores')
            .update({ allow_scheduling: checked })
            .eq('id', storeId)
        if (error) {
    ...
    const filteredAppointments = useMemo(() => {
        if (showPending) return storeAppointments
        return storeAppointments.filter((a) => a.status !== 'pending')
    ...
    return (
        <div className="mb-6 mt-4">
            ...
            >
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <label className="relative inline-flex cursor-pointer" style={{ width: 48, height: 26 }}>
                                ...
                                <span
                                    ...
                                    style={{ background: allowScheduling ? accentColor : colors.border }}
                                />
                                <span
                                    className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 ${allowScheduling ? 'translate-x-[22px]' : 'translate-x-0'
                                        }`}
                                />
                            </label>
                            <span className="text-xs font-bold" style={{ color: allowScheduling ? '#10b981' : '#ef4444' }}>
                                {allowScheduling ? 'Sim' : 'Não'}
                            </span>
                        </div>
                        {!allowScheduling && (
                            <p className="text-xs mt-1.5" style={{ color: textSecondary }}>
                                Você ainda poderá ver sua agenda, mas os outros não poderão agendar compromissos com essa loja.
                            </p>
                        )}
                    </div>
                </div>
                ...
                {loading ? (
                    ...
                    >
                        <p className="text-sm" style={{ color: textSecondary }}>
                            {hasHiddenPending
                                ? 'Você tem pendentes ocultos. Ative a exibição para vê-los.'
                ...
                {/* Rodapé: "Ver agenda" + toggle "Mostrar pendentes" */}
                <div
