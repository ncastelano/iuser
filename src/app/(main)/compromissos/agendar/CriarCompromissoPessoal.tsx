...
export default function CriarCompromissoPessoal({ onBack }: Props) {
    ...
    const slotsLivres = useMemo(() => {
        ...
        let relevantAppointments = appointments.filter(a => a.date === dateStr && a.status !== 'cancelled' && !a.store_id && (a.customer_id === userId || a.provider_profile_id === userId))
    ...
    const eventsByDate = useMemo(() => { const map: Record<string, number> = {}; appointments.filter(a => !a.store_id && (a.customer_id === userId || a.provider_profile_id === userId)).forEach(a => { map[a.date] = (map[a.date] || 0) + 1 }); return map }, [appointments, userId])
    ...
    const goBack = () => { if (step === 'confirm') setStep('datetime'); else onBack() }
    ...
    async function handleConfirm() {
        ...
        const appointment = { provider_profile_id: userId, date: dateStr, time: selectedTime, duration_minutes: selectedDuration, service_name: note, service_type: 'service', people_count: 1, customer_id: userId, customer_slug: slug, customer_avatar_url: myAvatar, owner_id: userId, owner_slug: slug, status: 'confirmed', direction: 'outgoing', is_public: isPublic }
        ...
        <main style={{ minHeight: '100vh', background: '#000', paddingBottom: 40, position: 'relative' }}>
            ...
            <div className="relative z-10">
                ...
                <div style={{ background: 'linear-gradient(135deg, #000, #000)', padding: '28px 24px', color: '#fff', borderBottomLeftRadius: 36, borderBottomRightRadius: 36, boxShadow: '0 10px 40px rgba(255,255,255,0.15)' }}>
                    ...
                    <h1 style={{ fontSize: 34, fontWeight: 800, marginTop: 16, letterSpacing: '-0.5px' }}>{step === 'datetime' ? 'Novo compromisso pessoal' : 'Confirmar'}</h1>
                    <p style={{ opacity: 0.7, marginTop: 6, fontSize: 15, fontWeight: 500 }}>{step === 'datetime' ? 'Compromisso privado' : 'Revise os detalhes'}</p>
                </div>
                ...
                <div style={{ padding: '20px 20px 0' }}>
                        ...
                        <>
                            <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 28, padding: 24, border: '1px solid rgba(255,255,255,0.1)', marginBottom: 24, backdropFilter: 'blur(10px)' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
                                    <div style={{ width: 52, height: 52, borderRadius: 16, background: 'linear-gradient(135deg, #10b981, #34d399)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}><Lock size={24} /></div>
                                    <div><p style={{ fontWeight: 800, fontSize: 18, color: '#fff' }}>Compromisso pessoal</p><p style={{ color: '#94a3b8', fontSize: 14 }}>Visível apenas para você</p></div>
                                </div>
                                <input type="text" value={appointmentNote} onChange={(e) => setAppointmentNote(e.target.value)} placeholder="Descrição do compromisso (opcional)" style={{ width: '100%', padding: '14px 18px', borderRadius: 16, border: '1px solid rgba(255,255,255,0.2)', fontSize: 15, outline: 'none', background: 'rgba(255,255,255,0.1)', color: '#fff' }} />
                                <div style={{ marginTop: 16 }}><label style={{ fontWeight: 700, fontSize: 14, color: '#94a3b8', display: 'block', marginBottom: 8 }}>Duração</label><select value={selectedDuration} onChange={(e) => setSelectedDuration(Number(e.target.value))} style={{ width: '100%', padding: '12px 16px', borderRadius: 14, border: '1px solid rgba(255,255,255,0.2)', fontSize: 15, outline: 'none', background: 'rgba(255,255,255,0.1)', color: '#fff' }}><option value={15}>15 minutos</option><option value={30}>30 minutos</option><option value={45}>45 minutos</option><option value={60}>1 hora</option><option value={90}>1h30</option><option value={120}>2 horas</option><option value={180}>3 horas</option><option value={240}>4 horas</option></select></div>
                            </div>
                    ...
                    {step === 'confirm' && selectedDate && selectedTime && (
                        <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 28, padding: 28, border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)' }}>
                            <div style={{ width: 72, height: 72, borderRadius: 20, background: 'linear-gradient(135deg, #10b981, #34d399)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 24px', boxShadow: '0 10px 30px rgba(16,185,129,0.25)' }}><Lock size={34} color="#fff" /></div>
                            <h2 style={{ textAlign: 'center', fontWeight: 800, fontSize: 24, color: '#fff', marginBottom: 12 }}>{appointmentNote || 'Compromisso pessoal'}</h2>
